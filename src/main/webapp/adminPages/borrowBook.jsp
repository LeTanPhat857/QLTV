...
<div class="content" style="padding-top: 5px">
	...
	<div class="d-flex flex-row justify-content-around"
		...
		<div class="info d-flex flex-row justify-content-center p-2"
			...
			<div
				...
				<span>Họ và tên: ${reader.name}</span> <span>Mã thẻ:
					...
					sinh: <fmt:formatDate pattern="dd/MM/yyyy" value="${reader.birthday}" /> </span> <span>Giới tính: ${reader.gender}</span> <span>Trạng
		...
		<div class="addBook d-flex"
			...
			<div style="width: 90%; margin: auto;">
				<input id="barcode" type="text" placeholder="Nhập mã sách" onkeypress="addBook_keyEvent()" autofocus
		...
		<table class="table text-center" style="margin-bottom: 0px">
			...
			<tbody>
				<c:forEach var="book" items="${borrowedBooks}">
					<tr>
						<td>Đang mượn</td>
						<td><fmt:formatDate pattern="HH:mm:ss dd/MM/yyyy" value="${book.date}"/></td>
						<td>${book.barcode}</td>
						<td>${book.title}</td>
						<td>${book.author}</td>
						<td>${book.publisher}</td>
						<td><fmt:formatNumber type="number" maxFractionDigits="0" value="${book.price}"/>  đ</td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
	...
	<div class="d-flex flex-row justify-content-center mt-3">
		<button id="borrowButton" class="btn btn-primary pr-5 pl-5" onclick="borrowBook()"
			style="margin-right: 80px;">Mượn sách</button>
		<button id="resetButton" type="reset" class="btn btn-primary pr-5 pl-5" onclick="cancelBorrow()"
